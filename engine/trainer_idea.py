...
def create_supervised_trainer(model, optimizer, loss_fn, loss_attention_cri, with_closs,
    ...
    def _update(engine, batch):
        ...
        img_aug = F.upsample_bilinear(img, [336, 112])#288,96
        score_img, feat_img, _, _ = model(img, target)
        score_aug, feat_aug, _, _ = model(img_aug.flip(-1), target)
        loss = loss_fn(score_img, feat_img, target) + loss_fn(score_aug, feat_aug, target)
        loss /= 2
        
        loss.backward()
        optimizer.step()
        # compute acc
        acc = ((score_img.max(1)[1] == target).float().mean() \
            + (score_aug.max(1)[1] == target).float().mean()) / 2
